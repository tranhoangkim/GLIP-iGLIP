...
caption = 'red car'
threshold = 0.64
DATA_DIR = f'/cm/shared/kimth1/Tracking/ByteTrack/datasets/GMOT_40/test'
SAVE_DIR = f'./ablation_threshold/{caption}_{threshold}/vis_before'
DROP_SAVE_DIR = f'./ablation_threshold/{caption}_{threshold}/vis_after'
BOX_SAVE_DIR = f'./ablation_threshold/{caption}_{threshold}/GMOT_40_boxes'
...
def get_object_in_image(glip_demo, caption, threshold, frame_name, img_folder, save_vis_before, save_vis_after):
    ...
    # before post-proc 
    result = draw_box(result, [469,732-550,1469,732-550+700+50])
    cv2.imwrite(save_path, result)
    
    # after post-proc
    nms_output = nms(boxes=pred_boxes.bbox, scores=pred_boxes.get_field("scores"), iou_threshold=0.8)
